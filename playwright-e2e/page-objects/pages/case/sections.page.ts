...
class SectionsPage extends Base {
  ...
  constructor(page) {
    ...
    this.createSectionLink = page
      .getByRole("link", { name: "Create New Section" })
      .first();
  }
  ...
  async uploadAndValidateUnrestrictedSectionDocument(
    ...
    try {
      await unrestrictedDocument.waitFor({ state: "visible", timeout: 30000 });
    } catch (error) {
   ...
   */
  async uploadUnrestrictedSectionDocument(key: string, filename: string) {
    await this.goToUploadDocuments(key);
    await this.uploadDocumentPage.uploadUnrestrictedDocument(filename);
  }
